...
import { getClientModelInfo } from '@/lib/ai/client-models';
...
function PureMultimodalInput({
  ...
  const modelResolver = useMemo(() => {
    const modelInfo = getClientModelInfo(selectedModelId);
    return {
  ...
  const contextMax = useMemo(() => {
    // Get context window from client model config
    const modelInfo = getClientModelInfo(selectedModelId);
    const cw = getContextWindow(modelInfo.modelId);
    return cw.combinedMax ?? cw.inputMax ?? modelInfo.contextWindow;
  }, [selectedModelId]);
